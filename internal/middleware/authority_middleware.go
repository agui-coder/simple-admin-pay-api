...
func (m *AuthorityMiddleware) Handle(next http.HandlerFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		act := r.Method
		// get the role id
		roleIds := r.Context().Value(consts.RoleId).(string)
		...
		if result {
			logx.Infow("HTTP/HTTPS Request", logx.Field("UUID", r.Context().Value(consts.UserId).(string)),
				logx.Field("path", obj), logx.Field("method", act))
			next(w, r)
			return
		} else {
			logx.Errorw("the role is not permitted to access the API", logx.Field(consts.RoleId, roleIds),
				logx.Field("path", obj), logx.Field("method", act))
